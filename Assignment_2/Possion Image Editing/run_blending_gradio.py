...
def add_point(img_original, polygon_state, evt: gr.SelectData): # evt: gr.SeclectData Python中的 Annotation 是一种注解，用来提示变量的类型
    """
    ...
    x, y = evt.index 
    polygon_state['points'].append((x, y)) # 字典中添加元组

    img_with_poly = img_original.copy()
    draw = ImageDraw.Draw(img_with_poly) # Pillow库ImageDraw函数创建Draw对象

    # Draw lines between points
    if len(polygon_state['points']) > 1:
        draw.line(polygon_state['points'], fill='red', width=2) # 画线条

    # Draw points
    for point in polygon_state['points']:
        draw.ellipse((point[0]-3, point[1]-3, point[0]+3, point[1]+3), fill='blue') # 画圆点
...
def create_mask_from_points(points, img_h, img_w):
    ...
    """
    mask1 = np.zeros((img_h, img_w,3), dtype=np.uint8)
    ### FILL: Obtain Mask from Polygon Points. 
    ### 0 indicates outside the Polygon.
    ### 255 indicates inside the Polygon.
    pts = points.reshape((-1,1,2)).astype(np.int32) # cv2.fillpoly必须使用int32的数据类型
    cv2.fillPoly(mask1,[pts],[255,255,255],1,0)
    mask=mask1[:,:,0]
    return mask
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    weight=torch.tensor([[[[0,-1,0],[-1,4,-1],[0,-1,0]]]],device=foreground_img.device).float().expand(3,-1,-1,-1)

    forground_img_lap=torch.nn.functional.conv2d(foreground_img,weight,padding='same',groups=3)
    blended_img_lap=torch.nn.functional.conv2d(blended_img,weight,padding='same',groups=3)

    mask_fg=foreground_mask.expand(-1,3,-1,-1).bool()
    mask_bg=background_mask.expand(-1,3,-1,-1).bool()

    loss_lap=torch.nn.L1Loss(reduce='sum')
    loss=loss_lap(blended_img_lap[mask_bg],forground_img_lap[mask_fg])
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    fg_img_tensor = torch.from_numpy(foreground_np).to(device).permute(2, 0, 1).unsqueeze(0).float() / 255. # permute(2,0,1),维度重排为（channel,height,width) [1,3,h,w]
    bg_img_tensor = torch.from_numpy(background_np).to(device).permute(2, 0, 1).unsqueeze(0).float() / 255. #.float() 由于torch.from_numpy默认转为float64 tensor,需要转为float32
    fg_mask_tensor = torch.from_numpy(foreground_mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255. # 插入两个长度为1的维度 [1,1,h,w]
    bg_mask_tensor = torch.from_numpy(background_mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255. # [1,1,h,w]
    ...
    blended_img[mask_expanded] = blended_img[mask_expanded] * 1 + fg_img_tensor[fg_mask_tensor.bool().expand(-1, 3, -1, -1)] * 0
    blended_img.requires_grad = True # 用于控制是否计算梯度，True 张量反向传播过程中计算梯度
